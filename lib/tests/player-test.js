...
describe('Player', () => {
    ...
    describe('Player on events from OOPlayer', () => {
        ...
        it('should invoke onLoadCallback on PLAYBACK_READY event', () => {
            // Media manager mock should do this trick
        ...
        it('should set playheadTime on PLAYHEAD_TIME_CHANGED event', () => {
            //referential eq?
            const playhead = {data: 1};
            OOPlayerMock.mb.publish(OO.EVENTS.PLAYHEAD_TIME_CHANGED, playhead);
            expect(player.isCompleted).toBeFalsy();
            const expectedPlayhead = { '0': OO.EVENTS.PLAYHEAD_TIME_CHANGED, '1': {data: 1} };
            expect(player.playhead).toEqual(expect.objectContaining(expectedPlayhead));
            expect(player.mb.broadcast).toHaveBeenCalledWith(expectedPlayhead);
        });
